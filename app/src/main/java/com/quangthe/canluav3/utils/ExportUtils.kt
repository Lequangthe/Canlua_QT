...
object ExportUtils {
    ...
    fun exportDatabaseToCsv(
        ...
        val exportDir = File(context.cacheDir, "exports").also { it.mkdirs() }
        val fileName = "Backup_CanLua_${SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())}.csv"
        val file = File(exportDir, fileName)
    ...
    fun exportMultipleToExcel(
        context: Context,
        data: List<Triple<RiceTicket, List<RiceSheet>, List<RiceCell>>>
    ) {
        val workbook = XSSFWorkbook()
        val gson = Gson()
        ...
        val exportDir = File(context.cacheDir, "exports").also { it.mkdirs() }
        val fileName = "Tong_Hop_Can_Lua_${System.currentTimeMillis()}.xlsx"
        val file = File(exportDir, fileName)
